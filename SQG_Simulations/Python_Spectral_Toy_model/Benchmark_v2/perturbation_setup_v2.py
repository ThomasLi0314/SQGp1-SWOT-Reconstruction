...
def perturb_A1(eta_tar, eta_tar_hat, kx, ky, Ro):
    """
...
def perturb_A2(eta_tar, eta_tar_hat, kx, ky, Ro):
    """
...
def perturb_A3(eta_tar, eta_tar_hat, K2, Ro):
    """
    ...
    return normalize_to_rms(zeta, eta_tar, Ro)


# ── Group B: Inertia-gravity waves ────────────────────────────────────────────

def perturb_B1(eta_tar, X, Y, Ro, k0=10.0, theta=jnp.pi / 4):
    """
    B.1 Plane wave
...
def perturb_B2(eta_tar, X, Y, Ro, rng_key):
    """
...
def perturb_C1(eta_tar, K, Ro, rng_key, K0=9.5):
    """
    C.1 White noise only at high k
    """
    Nx, Ny = eta_tar.shape
...
def perturb_C2(eta_tar, K, Ro, rng_key, K0=9.5):
    """
    C.2  White noise only at low k
    """
    print(K0)
    Nx, Ny = eta_tar.shape
    noise_hat = jnp.fft.fft2(jax.random.normal(rng_key, (Nx, Ny)))
...
def perturb_C3(eta_tar, K, Ro, rng_key):
    """
...
def perturb_E1_karin(eta_tar, kx, ky, Ro, rng_key, k_corr=10.0):
    """
...
def perturb_E2_roll(eta_tar, X, Y, Lx, Ly, Ro, rng_key,
                    n_along=3, n_cross=1):
    """
...
def build_all_perturbations(eta_tar, eta_tar_hat, kx, ky, K, K2, inv_K2,
    ...
    dict  {case_name: (perturbation, target)}
        ...
        target       : (Nx, Ny)   psi_true  if perturb_streamfunction=True
                       (Nx, Ny)   eta_true = eta_tar + p  if perturb_streamfunction=False
    """
    # Split into three keys:
    #   igw_key   → B2 (random IGW phases)
    #   noise_key → C1, C2, C3, E1 (shared white-noise draw, only the
    #               spectral shaping differs between cases)
    #   roll_key  → E2 (random low-k amplitudes/phases for roll proxy)
    igw_key, noise_key, roll_key = jax.random.split(rng_key, 3)

    perturbations = {
        "A1_nothing":         perturb_A1(
                                  eta_tar, eta_tar_hat, kx, ky, Ro),
        "A2_frontogenesis":   perturb_A2(
                                  eta_tar, eta_tar_hat, kx, ky, Ro),
        "A3_vorticity":       perturb_A3(
                                  eta_tar, eta_tar_hat, K2, Ro),
        "B1_mono_igw":        perturb_B1(
                                  eta_tar, X, Y, Ro),
        "B2_igw_packet":      perturb_B2(
                                  eta_tar, X, Y, Ro, igw_key),
        "C1_high_k_noise":    perturb_C1(
                                  eta_tar, K, Ro, noise_key),
        "C2_low_k_noise":     perturb_C2(
                                  eta_tar, K, Ro, noise_key, K0=7.0),
        "C3_k2_spectrum":     perturb_C3(
                                  eta_tar, K, Ro, noise_key),
        # ── Group E: SWOT-like errors (opt-in) ─────────────────────────
        "E1_karin":           perturb_E1_karin(
                                  eta_tar, kx, ky, Ro, noise_key, k_corr=10.0),
        "E2_roll":            perturb_E2_roll(
                                  eta_tar, X, Y, Lx, Ly, Ro, roll_key,
                                  n_along=3, n_cross=1),
    }
