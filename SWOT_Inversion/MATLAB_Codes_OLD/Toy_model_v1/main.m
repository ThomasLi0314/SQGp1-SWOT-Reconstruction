...
%% Code for Check
[X, Y, Z] = ndgrid(x, y, z);
F1_numerical = (exp(sqrt(2) * Z) - exp(2 * Z)) .* cos(X) .* sin(Y);
G1_numerical = (exp(2 .* Z) - exp( sqrt(2) .* Z)) .* cos(Y) .* sin(X);

[error_F1, idx_F1] = max(abs(F1_numerical - F1_true), [], 'all');
[i1, j1, k1] = ind2sub(size(F1_numerical), idx_F1);
fprintf('Max error F1: %e at linear index %d (%d, %d, %d)\n', error_F1, idx_F1, i1, j1, k1);

[error_G1, idx_G1] = max(abs(G1_numerical - G1_true), [], 'all');
[i2, j2, k2] = ind2sub(size(G1_numerical), idx_G1);
fprintf('Max error G1: %e at linear index %d (%d, %d, %d)\n', error_G1, idx_G1, i2, j2, k2);

disp(max(error_G1, error_F1));
