...
@jit
def vorticity_term(phi0_s_hat, mu, inv_mu, kx, ky, K, K2, Bu):
    """
    ...
    I_5 = jnp.fft.fft2(phi0_s_z * phi0_s_zz) * K
    ...
    zeta_s_hat = (I_1 + I_2 + I_3 + I_4 + I_5 - I_6 - I_7) / Bu # bug fixed!!!!
    return zeta_s_hat
...
def calculate_surface_u(phi0_s_hat, mu, inv_mu, kx, ky, K2, inv_K2, epsilon, Bu):
    ...
    Phi_1_term1 = jnp.fft.fft2(phi0_s_yz * phi0_s_z)

    phi0_s_zz = jnp.real(jnp.fft.ifft2(phi0_s_hat * mu * mu))
    Phi_1_term2 = jnp.fft.fft2(phi0_s_z * phi0_s_zz) * inv_mu

    Phi_1_term3 = jnp.fft.fft2(phi0_s_xz * phi0_s_z)

    Phi1_s_hat_y = Phi_1_term1 - 1j * ky * Phi_1_term2
    Phi1_s_hat_x = Phi_1_term3 - 1j * kx * Phi_1_term2
...
def forward_ssh(phi0_s_hat, kx, ky, mu, inv_mu, K2, inv_K2, Bu, epsilon,
    ...
    K = jnp.sqrt(K2)
    vort = vorticity_term(phi0_s_hat, mu, inv_mu, kx, ky, K, K2, Bu)
